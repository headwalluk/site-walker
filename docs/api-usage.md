...
- **Final-turn wind-down (M23.6, automatic).** When session spend is past 95% of the cap **or** the M23.5 sim hard-trigger is about to fire, a built-in **system directive** is appended to that turn's prompt — outside any `<block>` envelope, with a `--- DIRECTIVE FOR THIS TURN ---` sentinel and an explicit override of the block-handling rule. It tells the LLM that the visitor cannot reply and instructs it to close with a declarative statement (e.g. "I hope this helps.") rather than a follow-up question. Hardcoded for v1; no operator configuration required.
