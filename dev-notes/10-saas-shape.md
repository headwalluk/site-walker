...
**Provisioning key.** One per deployment. Lives in `.env` as `SW_PROVISIONING_KEY` — a 32-byte base64 value generated by `sw secrets gen-provisioning-key` and pasted into `.env` by the operator. At boot, hashed once (sha-256); on every `POST /admin/accounts*` request the incoming bearer is hashed and compared against the in-memory hash via `crypto.timingSafeEqual`. Lets the holder call `POST /admin/accounts` and `POST /admin/accounts/{id}/keys`. Does **not** grant access to the contents of any account.

- **Unset is a valid configuration.** Self-hosters who use the local CLI for everything never set this, and `/admin/accounts*` returns `503 no_provisioning_key` to any caller. The env var only matters for SaaS deployments where `site-walker-for-woo` calls the API server-to-server.
- **Boot validation fails loud** if `SW_PROVISIONING_KEY` is set to an empty string OR is shorter than 24 bytes of decoded entropy. Operators should not compose this by hand — the CLI helper is the supported path.
- **Rotation is restart-based, with a coordinated cutover.** Procedure: `sw secrets gen-provisioning-key` → paste the new value into `.env` → restart the API → update `site-walker-for-woo` with the new value. Between the restart and the WC-side update, the WC plugin's calls are rejected — accept the brief blip. There is no overlap-grace-period and no instant revocation; both are deliberately traded for the air-gap below.

**Account admin key.** One or more per account. Minted by either the provisioning key (SaaS path, via `POST /admin/accounts/{id}/keys`) or the CLI (self-hoster path, via `sw account add-admin-key <account-slug>` which inserts a hashed row and prints the raw key once — GitHub-PAT style). Stored in the `admin_keys` table with `account_id NOT NULL` — **the provisioning key is not in this table**; see the air-gap rationale below. Bearer-auth on `POST /admin/...`. Grants full control of **that account only** — chatbots, origins, persona, model selection, blocks, budgets, usage. Cannot see or touch other accounts.
...
### Why `SW_PROVISIONING_KEY` is not in `admin_keys`

Settled 2026-05-20. We considered storing the provisioning key as an `admin_keys` row with `account_id IS NULL` — symmetric storage, single auth code path, hash-at-rest for both key types, instant revocation. We chose the air-gap instead.

**The decisive argument:** physically separating the storage removes an entire class of risk. A buggy or compromised code path that touches `admin_keys` — say, a malformed `POST /admin/accounts/{id}/keys` that ends up writing a row with the wrong scope — cannot accidentally create a provisioning credential, because provisioning keys aren't in that table at all. The worst-case bug there mints a rogue *account admin* key, not a rogue *provisioning* key. Different blast radius for a credential at a different power level.

**What we trade for the air-gap:** no graceful rotation overlap (cutover is restart-based, with a coordinated update on `site-walker-for-woo`); no revocation without restart; two auth code paths on `/admin/*` routes (env-hash check, then DB lookup); no `last_used_at` audit on the provisioning key itself. Acceptable for v1 with a single SaaS deployment — rotation is a high-touch, low-frequency event, and the operational process is documented as part of the runbook when M19 ships.

**Do not "tidy this up"** by consolidating the provisioning key into `admin_keys` for code symmetry. The asymmetry is load-bearing.
