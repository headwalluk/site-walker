...
- **Visitor `country_code` capture.** Migration `0007_sessions_country_code.js` adds `sessions.country_code CHAR(2) NULL`. Captured at `POST /sessions` mint for **every** session: geo-restricted chatbots reuse the country the geo check already resolved; `allowall` (which short-circuits enforcement's lookup) gets one explicit `geoChecker.lookup(req.ip)`. NULL when unresolved (private/loopback IP, unindexed range, no GeoIP DB loaded). **The visitor IP is never stored** — only the 2-char ISO code. Pulls forward Option C from [`15-privacy-friendly-analytics.md`](15-privacy-friendly-analytics.md). Threaded via `createSession(db, chatbotId, { countryCode })`. Admin-mode sessions (`POST /admin/chatbots/{slug}/sessions`) stay NULL **by design**: they're minted server-to-server (the WP plugin's admin page calls the API from the WP server, then relays the token to the browser), so `req.ip` there is the WP server's datacenter IP, not the admin's location — capturing it would record something misleading rather than useful. The geo checker isn't even wired into the admin plugin. Confirmed live 2026-06-09: a public-path session captured the visitor's country; an admin-mode session correctly stored NULL.
...
**0.22.1 patch (2026-06-09):** `./bin/sw sessions list` gains a `country` column (2-char ISO, `--` for NULL); `listSessions` selects `s.country_code`. Display-only, no behaviour change. Surfaced while dogfooding the capture: a session minted through the website showed `--`, which traced to it being an **admin-mode** session — confirming the admin-mode-stays-NULL design (admin mint is server-to-server; `req.ip` is the WP server, not the admin). A public-path session captured the country correctly.
